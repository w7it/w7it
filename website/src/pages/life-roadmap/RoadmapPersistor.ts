...
export class RoadmapPersistor {
    ...
    public load(): void {
        ...
        const dto: DataTransferObject = JSON.parse(lz.decompress(str, { inputEncoding: 'Base64' }));
        ...
        const weeks = (dto['w'] ?? '').split('').map((char) => parseInt(char, 36));
    ...
    public save(): void {
        ...
        if (state.legend) {
            dto['l'] = state.legend.map((legend) => `${legend.color}${legend.label}`);
        }

        const result = lz.compress(JSON.stringify(dto), { outputEncoding: 'Base64' });
